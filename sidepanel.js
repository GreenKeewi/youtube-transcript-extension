...
function loadTranscript() {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.tabs.sendMessage(tabs[0].id, { action: 'requestTranscript' }, (response) => {
...
function setupEventListeners() {
  ...
  elements.aiPlatform.addEventListener('change', (e) => {
    ...
    elements.sendBtn.disabled = !e.target.value || !elements.transcriptText.value;
  });

  elements.transcriptText.addEventListener('input', () => {
    elements.sendBtn.disabled = !elements.aiPlatform.value || !elements.transcriptText.value;
  });
...
function sendToAI() {
  ...
  chrome.tabs.create({ url: config.url }, (tab) => {
    if (chrome.runtime.lastError) {
      showStatus('Error opening tab: ' + chrome.runtime.lastError.message, 'error');
      return;
    }
    ...
    setTimeout(() => {
      ...
      chrome.tabs.sendMessage(tab.id, {
        ...
        if (chrome.runtime.lastError) {
          ...
          showStatus('Opened ' + config.name + '. Please paste the transcript manually if needed.', 'info');
